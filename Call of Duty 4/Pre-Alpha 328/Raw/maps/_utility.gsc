...
{
 /#
 	assertEx( isalive( self ), "Attempt to set a flag on entity that is not alive or removed" );
...
flag_wait_any( flag1, flag2, flag3, flag4 )
{
	...
	for( ;; )
	{
		...
		flag_wait_any_proc( flag1, flag2, flag3, flag4 );
	}	
}
...
"OptionalArg: <ends_on_death> : The sound will be cut short if the entity dies."
...
play_sound_on_tag( alias, tag, ends_on_death )
{
	...
	{
		assertex( isalive( self ), "Did ends_on_death on a dead entity" );
...
get_guy_with_script_noteworthy_from_spawner( script_noteworthy )
{
	...
	spawn_failed( guy );
	assertEx( isalive( guy ), "Guy with script_noteworthy " + script_noteworthy + " failed to spawn." );
	return guy;
}
...
get_guy_with_targetname_from_spawner( targetname )
{
	...
	guy = spawner spawn_ai();
	spawn_failed( guy );
	assertEx( isalive( guy ), "Guy with targetname " + targetname + " failed to spawn." );
	return guy;
}
...
get_guys_with_targetname_from_spawner( targetname )
{
	...
	assertEx( spawners.size > 0, "Spawner with targetname " + targetname + " does not exist." );
...
autosave_now( optional_useless_string )
{
