...
loudspeaker()
{
	...
	level thread dialogue_loudspeaker("launchfacility_a_rul_highalert");
	...
	level thread dialogue_loudspeaker("launchfacility_a_rul_redspentznaz");
	...
	level thread dialogue_loudspeaker("launchfacility_a_rul_motherland");	

	flag_wait( "one_bmp_left" );
	//Russian Loudspeaker	
...
saw_sound_and_fx(iSound, eVent, bVentDrop)
{
	...
	self waittillmatch ( "single anim", "switch" );
...
vent_drop()
{
	...
	//thread play_sound_in_space( level.scr_sound["vent_fall"], self.origin);
	fTime = 2.5;
...
friendly_vent_think( aTeam, sNumber, eVent )
{
	...
	/*-----------------------
	DO SAW SEQUENCE IF APPROPRIATE
	-------------------------*/	
	if ( issubstr( self.rappelNode.targetname, "node_saw_team" ) )
	{
		...
		self.rappelNode thread anim_single_solo(self, "saw_" + iNumber );
		...
		if ( self.isSawDude == true )
			...
			//self.rappelNode thread anim_loop_solo (self, XXX???, undefined, "stop_idle");
	...
	eRope show();
	self.rappelNode thread anim_single_solo(self, sAnim_rappel_setup_to_stand);
	self.rappelNode anim_single_solo(eRope, sAnim_rappel_setup_to_stand);
...
player_rappel()
{
	...
	//eRope hide();
	//player_node anim_first_frame_solo( eRope, "rappel_for_player_start" );
	//eRope show();
	...
	//effect = spawnFx( getfx("rappel_objective"), obj_position.origin );
	//triggerFx( effect );
...
level_end()
{
	...
	missionsuccess("launchfacility_b", true);
}
...
{
	level.player playlocalsound(level.scr_sound[sDialogue]);
}
