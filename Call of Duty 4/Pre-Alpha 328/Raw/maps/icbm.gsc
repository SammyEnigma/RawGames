...
objectives()
{
	...
	objective_add( 2, "active", "Rescue Sgt.Griggs", obj.origin );
	objective_current( 2 );
...
patrol_guys()
{
	...
	flag_wait( "shutoff_flshlight" );
	patroller1 notify( "flashlight_off" );
...
moveto_driveway()
{
	...
	trigger waittill( "trigger" );		
    trigger trigger_off();
	array_thread( getentarray( "vision_inside", "targetname" ), ::vision_inside );
	array_thread( getentarray( "vision_outside", "targetname" ), ::vision_outside );
...
laundryroom_inside()
{
	...
	// INSIDE
	// set_vision_set( "icbm_interior", 1 );
...
second_floor_kill_counter()
{
	...
	level.buddies[ 1 ] anim_single_queue( level.buddies[ 1 ], "copythat" );
...
{	
	...
	// VISION LIGHTING CHANGE
	// INSIDE
	// set_vision_set( "icbm_interior", 2 );
...
house02_secondfloor_guys()
{
	...
	house02_bd02 thread magic_bullet_shield();
	flag_wait( "hq_entered" );
	house02_bd02 thread stop_magic_bullet_shield();
}
...
player_cut_grigs_loose()
{
   ...
   // level thread maps\_utility::play_sound_in_space( "intelligence_pickup", self.origin );    
	...
	level.mark setFlashBanged( false );
...
{	
	...
	for ( i = 0;i < level.buddies.size;i++ )
    {
		level.buddies[i] enable_cqbwalk();
		level.buddies[i] cqb_aim( mi_17_01 );
		//level.buddies allowedstances( "prone", "crouch" );
    }
              
	wait 1;
		
	level.price cqb_aim( mi_17_01 );
	level.mark cqb_aim( mi_17_01 );
	...
	for ( i = 0;i < level.buddies.size;i++ )
    {
		level.buddies[i] disable_cqbwalk();
		//level.buddies allowedstances( "prone", "crouch" );
    }
...
{	
	...
	// "tagetname" "leet_spetz"			
	axis = getaiarray( "axis" );
	for ( i = 0; i < axis.size; i++ )
	{
		if ( isdefined( axis[ i ].basecleaned ) )
			continue;
...
meetup_second_squad()
{
	...
	gm5_end_node thread anim_reach_and_approach_solo_set_flag( level.gm5, "icbm_end_sniper", "gm5_reached_end_anim" );
	
	price_end_node = getnode( "price_end_node", "script_noteworthy" );
	price_end_node thread anim_reach_and_approach_solo_set_flag( level.price, "icbm_end_price", "price_reached_end_anim" );
	
	flag_wait( "gm5_reached_end_anim" );
	flag_wait( "price_reached_end_anim" );
	
	gm5_end_node thread anim_single_solo( level.gm5, "icbm_end_sniper" );
	price_end_node thread anim_single_solo( level.price, "icbm_end_price" );
	
	level.gm5 anim_single_queue( level.gm5, "goodtosee" );
	activate_trigger_with_targetname( "turn_us_around" );
	level.gm5 set_force_color( "r" );
	level.price set_force_color( "b" );
