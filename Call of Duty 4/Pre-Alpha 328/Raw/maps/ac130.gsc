...
main()
{
	...
	wait 5;
	objective_add( 1, "active", &"AC130_OBJECTIVE_SUPPORT_FRIENDLIES", ( 0, 0, 0 ) );
}
...
gameplay_hijack()
{
	...
	if ( getdvar( "ac130_gameplay_enabled") == "1" )
	{
		...
		thread do_hijack( level.getaway_vehicle_1, vehGroup[ 0 ] );
		thread do_hijack( level.getaway_vehicle_2, vehGroup[ 1 ] );
...
dialog_ambush()
{
	...
	playSoundOverRadio( level.scr_sound["fco"]["ac130_fco_requestpermission"], true );
	
	// Uh, we got small groups taking up positions along the edge of the curved road. Cleared to engage those as well?
	playSoundOverRadio( level.scr_sound["fco"]["ac130_fco_smallgroups"], true );
